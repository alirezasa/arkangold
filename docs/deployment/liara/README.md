...
- **سرعت build**: `pnpm-lock.yaml` ریشه commit شده؛ بدون آن pnpm برای هر build اطلاعات همه‌ی پکیج‌های
  مونوریپو (حتی Next.js برای API) را از mirror لیارا می‌پرسد که هر درخواستش چند ثانیه طول می‌کشد. بعد از
  هر تغییر در وابستگی‌ها `pnpm install` را در ریشه اجرا کنید و فایل قفل را هم commit کنید.
